...
SDLCommonCreateState(char** argv, Uint32 flags)
{
  ...
  state = (SDLCommonState*)SDL_calloc(1, sizeof(*state));
  if (!state) {
    SDL_OutOfMemory();
    return NULL;
  }
...
int
SDLCommonArg(SDLCommonState* state, int index)
{
  ...
  if (SDL_strcasecmp(argv[index], "--trackmem") == 0) {
    /* Already handled in SDLCommonCreateState() */
    return 1;
  }
...
void
SDLCommonLogUsage(SDLCommonState* state, const char* argv0, const char** options)
{
  int i;
...
const char*
SDLCommonUsage(SDLCommonState* state)
{
...
SDL_bool
SDLCommonDefaultArgs(SDLCommonState* state, const int argc, char** argv)
{
  int i = 1;
  while (i < argc) {
    const int consumed = SDLCommonArg(state, i);
    if (consumed == 0) {
      SDLCommonLogUsage(state, argv[0], NULL);
      return SDL_FALSE;
    }
    i += consumed;
  }
...
static void
SDLPrintDisplayOrientation(char* text, size_t maxlen, SDL_DisplayOrientation orientation)
{
  switch (orientation) {
...
SDLPrintWindowFlags(char* text, size_t maxlen, Uint32 flags)
{
  ...
  for (i = 0; i < (sizeof(window_flags) / sizeof(window_flags[0])); ++i) {
    ...
    if ((flags & flag) == flag) {
      ...
      SDLPrintWindowFlag(text, maxlen, flag);
      ++count;
    }
  }
}
...
static void
SDLPrintRenderer(SDL_RendererInfo* info)
{
  int i, count;
  char text[1024];
  ...
  for (i = 0; i < sizeof(info->flags) * 8; ++i) {
    ...
    if (info->flags & flag) {
      ...
      SDLPrintRendererFlag(text, sizeof(text), flag);
      ++count;
    }
  }
  SDL_snprintfcat(text, sizeof(text), ")");
  ...
  for (i = 0; i < (int)info->num_texture_formats; ++i) {
    ...
    SDLPrintPixelFormat(text, sizeof(text), info->texture_formats[i]);
  }
  SDL_Log("%s\n", text);
...
SDLExampleHitTestCallback(SDL_Window* win, const SDL_Point* area, void* data)
{
  int w, h;
...
SDL_bool
SDLCommonInit(SDLCommonState* state)
{
  int i, j, m, n, w, h;
  ...
  if (state->flags & SDL_INIT_VIDEO) {
    ...
    for (i = 0; i < state->num_windows; ++i) {
      ...
      if ((state->window_flags & (SDL_WINDOW_RESIZABLE | SDL_WINDOW_BORDERLESS)) ==
        ...
        SDL_SetWindowHitTest(state->windows[i], SDLExampleHitTestCallback, NULL);
      }

      if (state->window_icon) {
        SDL_Surface* icon = SDLLoadIcon(state->window_icon);
        if (icon) {
...
static void
SDLScreenShot(SDL_Renderer* renderer)
{
  SDL_Rect viewport;
  SDL_Surface* surface;
...
void
SDLCommonEvent(SDLCommonState* state, SDL_Event* event, int* done)
{
  int i;
...
void
SDLCommonQuit(SDLCommonState* state)
{
  int i;
